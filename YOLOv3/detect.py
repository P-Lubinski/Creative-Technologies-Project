...
#load the image 
    ...
    for im_num, image in enumerate(imlist[i*batch_size: min((i +  1)*batch_size, len(imlist))]):
        ...
        for x in range(0,len(objs)):
            if (objs[x] == "table"):
                files = os.listdir("frames/tables/")
                files.sort()
                for current_file in files :
                    try:
                        with Image.open("frames/tables/" + current_file) as im :
                            string = current_file
                            substring = colourWanted
                            if substring in string:
                                frames.append(im.getdata())
                                print(colourWanted, " ", objs[x], " found")
                    except:
                        print(current_file + " is not a valid image")
    
            elif (objs[x] == "chair"):
                files = os.listdir("frames/chairs/")
                files.sort()
                for current_file in files :
                    try:
                        with Image.open("frames/chairs/" + current_file) as im :
                            string = current_file
                            substring = colourWanted
                            if substring in string:
                                frames.append(im.getdata())
            ...
            elif (objs[x] == "backpack"):
                files = os.listdir("frames/backpacks/")
                files.sort()
                for current_file in files :
                    try:
                        with Image.open("frames/backpacks/" + current_file) as im :
                            string = current_file
                            substring = colourWanted
                            if substring in string:
                                frames.append(im.getdata())
        ...
        print("Spritesheet height: ", spritesheet_height)
        print("Spritesheet width:  ", spritesheet_width)
